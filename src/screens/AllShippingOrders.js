...
const AllShippingOrders = () => {
    ...
    const renderShipmentItem = ({ item, index }) => (
    ...
    const renderEmptyComponent = () => (
        <View style={styles.noOrderContainer}>
            <Text style={styles.noOrderText}>No shipping load found</Text>
        </View>
    );

    const keyExtractor = (item, index) => {
        return item?.shipping_id ? item.shipping_id.toString() : index.toString();
    };
    ...
    return (
        <SafeAreaView style={styles.Container}>
            <CustomHeader commingFrom={'All Shipment Load'} onPress={() => navigation.goBack()} title={'All Shipment Load'} />
            <FlatList
                data={allShipmentList}
                ...
                getItemLayout={(data, index) => ({
                    length: responsiveHeight(56.4), // Approximate height of each item
                    offset: responsiveHeight(56.4) * index,
                    index,
                })}
            />
        </SafeAreaView>
...
export default AllShippingOrders
