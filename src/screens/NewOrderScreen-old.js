...
const NewOrderScreen = () => {
    ...
    const handleCancelConfirm = () => {
        ...
        executeDeclineOrder(currentOrderId, currentOrderType, reason);
        setShowCancelModal(false);
        setSelectedReason('');
        setCustomReason('');
    }

    const executeDeclineOrder = (orderId, type, reason) => {
        setIsLoading(true)
        const myArr = []
        const option = {};
        if (type == "batch") {
                                ...
                                <View style={{ flexDirection: 'row', paddingRight: 20 }}>
                                    {
                                        ...
                                        item?.status === 'Accepted' ?
                                            ...
                                            item?.status === 'Active' ?
                                                <>
                                                    <TouchableOpacity onPress={() => acceptSingleOrder(item?.order_item_id, "single")}>
                                                        <Image source={acceptImg} style={styles.iconImage2} />
                                                    </TouchableOpacity>
                                                    <TouchableOpacity onPress={() => declineSingleOrder(item?.order_item_id, "single")}>
                                                        <Image source={declineImg} style={styles.iconImage2} />
