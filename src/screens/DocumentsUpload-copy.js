...
const DocumentsUpload = ({ route }) => {
    ...
    const pickDocument = async (forwhat) => {
        try {
            ...
            if (forwhat == 'DrivingLicenseFront') {
                ...
                fileUpload('DrivingLicenseFront',result[0].uri)
    ...
    const fileUpload = (forwhat,imageUri) => {
        
        if (forwhat === 'DrivingLicenseFront') {
            const formData = new FormData();
            console.log(imageUri,'kkkk')
            ...
            .catch(e => {
                ...
                Alert.alert('Oops..', e.response.data?.response.records.message || 'An error occurred', [
                    {
                        text: 'OK',
                        onPress: () => console.log('OK Pressed'),
                        style: 'cancel',
