...
import { View, Text, SafeAreaView, StyleSheet, ScrollView, Dimensions, Image, TouchableOpacity, Alert, Platform, PermissionsAndroid } from 'react-native'
...
const ShippingLocationConfirmation = ({ route }) => {
    ...
    const checkLocationPermission = async () => {
        if (Platform.OS === 'ios') {
            // iOS will prompt for permission automatically when getCurrentPosition is called
            return true;
        } else {
            try {
    ...
    const getLocationAndUpdatePosition = async () => {
        if (isFetchingLocation) return; // Prevent overlapping requests
        ...
        try {
            ...
            setIsFetchingLocation(false); // Reset flag
        }
    };
    ...
    useEffect(() => {
        ...
        const setupLocation = async () => {
            await getLocationAndUpdatePosition(); // Initial location fetch

            if (isMounted) {
                interval = setInterval(getLocationAndUpdatePosition, 10000); // Run every 10 seconds
            }
        };
    ...
    const handleGetDirections = (destinationLat, destinationLong) => {
        if (!locationReady) {
            Toast.show({
                type: 'info',
                text1: 'Please wait',
                text2: 'Getting your current location...',
                position: 'top',
            });
            return;
        }
                ...
                buttonStatus ?
                    <View style={{ marginBottom: responsiveHeight(12), position: 'absolute', bottom: 0, width: '90%', alignSelf: 'center' }}>
                        <CustomButton label={"I have arrived"}
                            buttonColor={'active'}
                            onPress={() => navigation.navigate('ShippingItemVerifiedScreen', { orderId: getorders[0]?.id, forWhich: getorders[0]?.other.user_primary_id, itemId: getorders[0]?.id, orderType: orderType })}
                        />
                    </View>
                    :
                    <View style={{ marginBottom: responsiveHeight(12), position: 'absolute', bottom: 0, width: '90%', alignSelf: 'center' }}>
